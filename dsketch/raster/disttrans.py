...
@attrs(param_dim=4)
def catmull_rom_spline(params: torch.Tensor, t: torch.Tensor, alpha: float = 0.5) -> torch.Tensor:
...
def curve_edt2_bruteforce(params: torch.Tensor, grid: torch.Tensor, iters: int, slices: int,
                          cfcn: Callable[[torch.Tensor, torch.Tensor], torch.Tensor] = cubic_bezier,
                          mint: Union[float, torch.Tensor] = 0, maxt: Union[float, torch.Tensor] = 1,
...
def create_curve_edt2_bruteforce(iters: int, slices: int,
                                 cfcn: Callable[[torch.Tensor, torch.Tensor], torch.Tensor] = cubic_bezier,
                                 mint: Union[float, torch.Tensor] = 0, maxt: Union[float, torch.Tensor] = 1) \
...
def curve_edt2_polyline(params: torch.Tensor, grid: torch.Tensor, segments: int,
                        cfcn: Callable[[torch.Tensor, torch.Tensor], torch.Tensor] = cubic_bezier):
    """Batched version of a Squared Euclidean Distance Transform for a parameterised curve, c(t) for 0≤t≤1.
